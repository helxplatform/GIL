...
def model_config():
    ...
    if args.arch.lower() not in model_dict:
        ...
        model_arch = model_dict[args.arch.lower()]

    return args, model_arch

def split_and_resize(images, labels, test_ratio, input_shape=(1, 512, 512, 1), auto_resize=False, index_first=False, log=None):
    train_images, test_images, train_labels, test_labels = train_test_split(images, labels, test_size=test_ratio, random_state=42)

    training_set = VoxelSet(images=train_images, labels=train_labels, mode="segment", index_first=index_first)
    validation_set = VoxelSet(images=test_images, labels=test_labels, mode="segment", index_first=index_first)
    ...
    min_depth = min([training_set.min_depth, validation_set.min_depth])

    # Set input image shape
    if auto_resize:
        input_shape = (min_depth, min_height, min_width, 1) # (depth, height, width, channels)
...
def main():
    ...
    if base_model is None:
        print(f"{ARGS.arch} not in available models!")
