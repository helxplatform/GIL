...
class VoxelSet:
    """A 3D image stack dataset for batch generation in TensorFlow"""
    def __init__(self, images, labels, mode, input_shape=None, index_first=False):
        """
        Constructs an ImageSet from a list of stack files.
        ...
        Attributes:
            ...
            mode : str
                Model mode (classify, segment)
            input_shape : tuple
                (depth, height, width, channel) of 3D image
        """
        self.images = images
        self.labels = labels
        self.input_shape = input_shape
    ...
    def get_sizes(self):
        ...
        for image in self.images:
            ...
            if min_depth is None:
                min_depth = img_shape[depth_index]
            elif img_shape[depth_index] < min_depth:
                min_depth = img_shape[depth_index]
    ...
    def process_next_stack(self, file_index, height, width, depth):
        # Load stack file
        img = sitk.ReadImage(self.images[file_index])
        img_array = sitk.GetArrayFromImage(img)
        img_array = tf.image.resize(img_array, [height,width]).numpy()
        ...
        #    img_array[i] = tf.image.resize(temp_array[i], [height,width]).numpy()
        ...
        if self.mode == "classify":
            ...
            #for i in range(temp_array.shape[0]):
            #    img_label[i] = tf.image.resize(temp_label[i], [height,width]).numpy()

        while img_array.shape[0] < depth:
            np.append(img_array, np.zeros((1, img_array.shape[1], img_array.shape[2])), axis=0)
            np.append(img_label, np.zeros((1, img_label.shape[1], img_label.shape[2])), axis=0)

        if img_array.shape[0] > depth:
            depth_diff = img_array.shape[0] - depth
            top_crop = depth_diff // 2
            bottom_crop = depth_diff - top_crop
            img_array = img_array[top_crop + 1:bottom_crop]
            img_label = img_label[top_crop + 1:bottom_crop]
